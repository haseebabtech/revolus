...
if ($f == 'posts') {
    ...
    if ($s == 'load_more_posts') {
        ...
        if (!empty($_GET['filter_by_more']) && $_GET['filter_by_more'] == 'story' && isset($_GET['story_id']) && isset($_GET['user_id'])) {
            ...
            foreach ($most_liked_posts as $wo['story']) {
                if ($is_api == true) {
                    ...
                    echo sanitize_output(Wo_LoadPage('story/contentV2'));
                }
            }
     ...
     if ($s == 'load_more_postsV2') {
        ...
        if (!empty($_GET['filter_by_more']) && $_GET['filter_by_more'] == 'story' && isset($_GET['story_id']) && isset($_GET['user_id'])) {
        ...
        } else if (!empty($_GET['filter_by_more']) && !empty($_GET['after_post_id'] || !empty($_GET['before_post_id']))) {
            $page_id  = 0;
            $group_id = 0;
            $user_id  = 0;
            $story_id = 0;
            ...
            $get_posts = Wo_GetPosts2($postsData);
            $is_api    = false;
            if (!empty($_GET['is_api'])) {
                $is_api = true;
            }
    ...
    if ($s == 'load_more_postsV3') {
        ...
        if (!empty($_GET['filter_by_more']) && $_GET['filter_by_more'] == 'story' && isset($_GET['story_id']) && isset($_GET['user_id'])) {
            ...
            $get_posts = Wo_GetPosts3($postsData);
            $is_api    = false;
            if (!empty($_GET['is_api'])) {
                $is_api = true;
            }
            ...
            foreach ($get_posts as $wo['story']) {
                if ($is_api == true) {
                    ...
                    echo sanitize_output(Wo_LoadPage('story/contentV3'));
                }
            }
        }
        exit();
    ...
    if ($s == 'show_story') {
        ...
        if (!empty($_GET['filter_by_more']) && $_GET['filter_by_more'] == 'story' && isset($_GET['story_id']) && isset($_GET['user_id'])) {
            ...
            $get_posts = Wo_GetPosts4($postsData);
            $is_api    = false;
            if (!empty($_GET['is_api'])) {
                $is_api = true;
            }
    ...
    if ($s == 'register_reaction') {
        ...
        if (!empty($_GET['post_id']) && !empty($_GET['reaction'])) {
     ...
     if ($s == 'register_reactionV2') {
      
        if (!empty($_GET['post_id']) && !empty($_GET['reaction'])) {
      
            if (Wo_AddReactionsV2($_GET['post_id'], $_GET['reaction'], $_GET['image_id']) == 'reacted') {
                $data = array(
                    'status' => 200,
                    'reactions' => Wo_GetPostReactions($_GET['post_id']),
                    'like_lang' => $wo['lang']['liked']
    ...
    if ($s == 'register_comment') {
        ...
        if (!empty($_POST['post_id']) && isset($_POST['text'])) {
            ...
            $page_id = '';
            $image_id = 0;
            isset($_POST['image_id']) && ($_POST['image_id'] == 'undefined') ? $image_id = 0 : $image_id = $_POST['image_id']; 
    ...
    if( $s == 'register_story_comment'){
       if (!empty($_POST['story_id']) && isset($_POST['text'])) {
            $html    = '';
            $page_id = '';
            $text_comment = '';
            if (!empty($_POST['text']) && !ctype_space($_POST['text'])) {
            ...
            $R_Comment     = Wo_RegisterPostCommentV2($C_Data);
            return $R_Comment;
        ...
        if (!empty($_GET['filter_by']) && isset($_GET['id'])) {
            ...
            if (count($stories) > 0) {
                foreach ($stories as $wo['story']) {
                    $html .= Wo_LoadPage('story/contentV2');
                }
            } else {
                $html .= Wo_LoadPage('story/filter-no-stories-found');
            }
