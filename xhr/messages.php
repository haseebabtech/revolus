...
if ($f == 'messages') {
    ...
    if ($s == 'get_user_messages') {
        if (!empty($_GET['user_id']) AND is_numeric($_GET['user_id']) AND $_GET['user_id'] > 0 && Wo_CheckMainSession($hash_id) === true) {
            ...
            $recipient_id = !empty($recipient['user_id']) ? $recipient['user_id'] : 0;
    ...
    if($s == 'send_reply'){
        ...
        if ($messages > 0) {
            $data = ['success' => true,'publisher_id' => $_GET['user_id']];
            header("Content-type: application/json");
            echo json_encode($data);
            exit();
        }
    ...
    if ($s == 'update_message') {
        $data = ['status' => 0, 'msg' => ''];
        if (isset($_GET['message_id']) && isset($_GET['message_text']) && Wo_CheckMainSession($hash_id) === true) {
            $message_id = Wo_Secure($_GET['message_id']);
            $message_text = Wo_Secure($_GET['message_text']);
            ...
            if (!empty($message_id) || is_numeric($message_id) || $message_id > 0) {
                if (!empty($message_text)) {
                    ...
                    if($status === true){
                        $data = ['status' => 200, 'msg' => 'updated'];
                    } else{
                        $data = ['status' => 500, 'msg' => 'Error found '.$status];
                    }
                } else{
                    $data = ['status' => 500, 'msg' => 'Message Text cannot be empty'];
                }
            } else{
                $data = ['status' => 500, 'msg' => 'Message ID should be a numeric'];
            }
        }
        header("Content-type: application/json");
        echo json_encode($data);
    ...
    // FOR GROUP CHAT UPDATE
    if ($s == 'update_group_message') {
        $data = ['status' => 0, 'msg' => ''];
        if (isset($_GET['message_id']) && isset($_GET['message_text']) && Wo_CheckMainSession($hash_id) === true) {
            $message_id = Wo_Secure($_GET['message_id']);
            $message_text = Wo_Secure($_GET['message_text']);
