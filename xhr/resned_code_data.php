...
if ($f == 'resned_code_data') {
    if (isset($_POST['phone_number'])) {
        ...
        /*if (empty($errors)) {*/
       /* $result             = mysqli_query($sqlConnect, "SELECT " . T_USERS . "  WHERE `phone_number` = {$phone_number}");
        ...
        foreach($val as $value){
            ...
            $query             = mysqli_query($sqlConnect, "UPDATE " . T_USERS . " SET `sms_code` = '{$random_activation}' WHERE `phone_number` = {$phone_number}");
            if ($query) {
