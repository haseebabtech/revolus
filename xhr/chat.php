...
if ($f == 'chat') {
    ...
    if ($s == 'update-glanguage') {

        $usr_upd = Wo_UpdateUserData($wo['user']['user_id'], array(
            'glanguage' => $_GET['language_val']
        ));
        ...
        if($usr_upd){
            ...
            $data = array(
                'status' => 200,
                'is_success' => 1,
                'msg' => 'User Language updated successfully UID: '.$user['glanguage'].', Lang: '.$_GET['language_val']
            );
        } else{
            $data = array(
                'status' => 200,
                'is_success' => 0,
                'msg' => 'Updation Failed: UID: '.$wo['user']['user_id'].', Lang: '.$_GET['language_val']
            );
        }
    ...
    if ($s == 'group_participant_image'){
        if(empty($_GET['participant_identity'])){
            return false;
        }
        
        $data = ['exists' => 0, 'img' => ''];
        $getid = explode('|', $_GET['participant_identity']);
        if(isset($getid[0])){
            $user_id = Wo_Secure($getid[0]);
            $user   = Wo_UserData($user_id);

            if(!empty($user['avatar'])){
                $data = ['exists' => 1, 'img' => $user['avatar']];
            }
        }
    ...
    if ($s == 'chat_side') {
        if (Wo_CheckMainSession($hash_id) === true) {
            ...
            // if(isset($data['gp_user_id']) && !empty($data['gp_user_id'])){
            //     $ag = Wo_GetCurrentUserActiveGroups($data['gp_user_id']);
            //     if(isset($ag['exists']) && $ag['exists'] == 1 && isset($ag['data'])){
            //         foreach($ag['data'] as $dt){
            //             if($dt['from_id'] == $data['gp_user_id']){ //&& $dt['from_id'] != $_GET['to_id']
            //                 $data['gp_from_id'] = $dt['from_id'];
            //                 $data['gp_to_id'] = $dt['to_id'];
            //                 $data['gp_group_id'] = $dt['group_id'];
            //                 $data['gp_group_name'] = $dt['group_name'];
            //                 $data['gp_room_name'] = $dt['room_name'];
            //                 $data['gp_active'] = $dt['active'];
            //                 $data['gp_called'] = $dt['called'];
            //                 $data['gp_time'] = $dt['time'];
            //                 $data['gp_declined'] = $dt['declined'];
            //             }
            //         }
            //     }
            // }
            
            // GROUP VIDEO MODALBOX ENDS
            ...
            if (!empty($_GET['user_id']) && isset($_GET['message_id'])) {
                ...
                if (!empty($user_id)) {
                    ...
                    if (count($messages) > 0) {
                        // Recipient ID
                        $wo['recipient_id'] = $wo['user']['user_id'];
    ...
    if ($s == 'load_chat_messages') {
        if (!empty($_GET['recipient_id']) && is_numeric($_GET['recipient_id']) && $_GET['recipient_id'] > 0 && Wo_CheckMainSession($hash_id) === true) {
            ...
            // Recipient ID
            $wo['recipient_id'] = $recipient_id;
            
            foreach ($messages as $wo['chatMessage']) {
                $html .= Wo_LoadPage('chat/chat-list');
            }
            $html .= "<div class='quote_msg' id='quote_msg_".$recipient_id."'></div>";
            $data = array(
                'status' => 200,
                'messages' => $html
            );
        ...
        if ($wo['config']['who_upload'] == 'pro' && $wo['user']['is_pro'] == 0 && !Wo_IsAdmin() && (!empty($_FILES['sendMessageFile']) || !empty($_POST['message-record']))) {
            ...
            if ($wo['user']['message_privacy'] != 2) {
                if (!empty($_POST['user_id']) && Wo_CheckMainSession($hash_id) === true) {
                    ...
                    $quote_message = '';
                    if (!empty($_POST['quote_msg_txt'])) {
                        $quote_message = $_POST['quote_msg_txt'];
                    }
    ...
    if ($s == 'share_location') {
        if ($wo['user']['message_privacy'] != 2) {
            ...
            if (!empty($_POST['user_id']) && Wo_CheckMainSession($hash_id) === true) {
                ...
                $messages = Wo_RegisterMessageV3(array(
                    'from_id' => Wo_Secure($wo['user']['user_id']),
                    'to_id' => Wo_Secure($_POST['user_id']),
                    'text' => null,
                    'formatted_address' => $_POST['formatted_address'],
                    'location_link' => Wo_Secure($message_text),
                    'is_location_shared' => 1,
                    'time' => time(),
                ));
     ...
     if ($s == 'send_message_to_multiple') {
        ...
        if ($wo['user']['message_privacy'] != 2) {
            ...
            $mediaName     = '';
            $invalid_file  = 0;
           foreach($_POST['selectedUsers'] as $index=>$d){
               $message_text = '';
                if (!empty($_POST['selectedUsersMessage'])) {
                    $message_text = $_POST['selectedUsersMessage'];
                }
                $messages = Wo_RegisterMessageV3(array(
                    'from_id' => Wo_Secure($wo['user']['user_id']),
                    'to_id' => $d,
