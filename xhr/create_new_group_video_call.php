...
if ($f == 'create_new_group_video_call') {
    ...
    // User Declined Groupd Video Call
    if(isset($_GET['is_attended']) && $_GET['is_attended'] == 'declined'){
        Wo_UpdateUserCallAttend(Wo_Secure($wo['user']['user_id']), $_GET['group_id'], 'declined');
        $data = array(
            'status' => 200,
            'msg' => 'declined'
        );
    ...
    try {
        ...
        $group_members = Wo_GroupGetMembers($_GET['group_id']);
        
        $inserted_ids = [];
        if(count($group_members) > 0){
            foreach($group_members as $gm){
                $insertData = Wo_CreateNewGroupVideoCall(array(
        ...
        if(isset($_GET['is_attended']) && $_GET['is_attended'] == 'attended'){
            Wo_UpdateUserCallAttend(Wo_Secure($wo['user']['user_id']), $_GET['group_id']);
        }
        ...
        $data = array(
            ...
            'html' => Wo_LoadPage('modals/video_group_calling'),
