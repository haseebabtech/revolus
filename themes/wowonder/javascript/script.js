...
function Wo_OpenAlbumLightBox(image_id, type,postIds=null) {
    var albumids = null;
    if(postIds.length > 0) {
        //let postIds = postIds;
        albumids = formatPostIds(postIds);
        //alert(albumids);
        
    }
  $('body').append('<div class="lightbox-container"><div class="lightbox-backgrond" onclick="Wo_CloseLightbox();"></div><div class="lb-preloader" style="display:block"><svg width="50px" height="50px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid"><rect x="0" y="0" width="100" height="100" fill="none" class="bk"></rect><circle cx="50" cy="50" r="40" stroke="#676d76" fill="none" stroke-width="6" stroke-linecap="round"><animate attributeName="stroke-dashoffset" dur="1.5s" repeatCount="indefinite" from="0" to="502"></animate><animate attributeName="stroke-dasharray" dur="1.5s" repeatCount="indefinite" values="150.6 100.4;1 250;150.6 100.4"></animate></circle></svg></div></div>');
  $.get(Wo_Ajax_Requests_File(), {f:'open_album_lightbox', image_id:image_id, type:type,post_ids:albumids}, function(data) {
    if (data.status == 200) {
...
function Wo_OpenLightBox(post_id,postIds=null) {
    var albumids = null;
    //alert(post_id);
    if(post_id.length > 0) {
        //let postIds = postIds;
        albumids = formatPostIds(post_id);
        //alert(albumids);
        
    }
  $('#contnet').append('<div class="lightbox-container"><div class="lightbox-backgrond" onclick="Wo_CloseLightbox();"></div><div class="lb-preloader" style="display:block"><svg width="50px" height="50px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid"><rect x="0" y="0" width="100" height="100" fill="none" class="bk"></rect><circle cx="50" cy="50" r="40" stroke="#676d76" fill="none" stroke-width="6" stroke-linecap="round"><animate attributeName="stroke-dashoffset" dur="1.5s" repeatCount="indefinite" from="0" to="502"></animate><animate attributeName="stroke-dasharray" dur="1.5s" repeatCount="indefinite" values="150.6 100.4;1 250;150.6 100.4"></animate></circle></svg></div></div>');
  $.get(Wo_Ajax_Requests_File(), {f:'open_lightbox', post_id:post_id,post_ids:albumids}, function(data) {
      //alert(data.html);
...
function Wo_SwitchCall(id='') {
  ...
  $.get(Wo_Ajax_Requests_File(), {f:'cancel_call'}, function (data) {
    if (data.status == 200) {
      ...
      Wo_GenerateVideoCallV2();
      //Wo_GenerateVideoCall(412, 577);
      $("#btnvideoicn").click();
    }
  });
}
...
function Wo_GenerateVideoCallV2() {
...
// Group Video Call
function Wo_GenerateGroupVideoCall(user_id1, group_id, group_name, is_attended="no_call_attend"){
    $.get(Wo_Ajax_Requests_File(), {f:'create_new_group_video_call', 'new': 'true', user_id1: user_id1, group_id: group_id, group_name: group_name, is_attended:is_attended}, function(data) {
        if (data.status == 200) {
            console.log(data);

            if(is_attended == "declined"){
                $("#video-group-modal").modal("hide");
                return false;
            }
          
            //console.log('https://revolus.com/video-group-call?access_token='+data.accessToken+'&room_name='+data.roomName);
            window.location = data.url+'/video-group-call?access_token='+data.accessToken+'&room_name='+data.roomName+'&link1=video-group-call';
        }
    });
}

// Group Audio Call
function Wo_GenerateGroupAudioCall(user_id1, group_id, group_name, is_attended="no_call_attend"){
    $.get(Wo_Ajax_Requests_File(), {f:'create_new_group_audio_call', 'new': 'true', user_id1: user_id1, group_id: group_id, group_name: group_name, is_attended:is_attended}, function(data) {
        if (data.status == 200){
            console.log(data);
            //return false;
            ...
            if(is_attended == "declined"){
                $("#audio-group-modal").modal("hide");
                return false;
            }
          
            //console.log('https://revolus.com/audio-group-call?access_token='+data.accessToken+'&room_name='+data.roomName);
            window.location = data.url+'/audio-group-call?access_token='+data.accessToken+'&room_name='+data.roomName+'&link1=audio-group-call';
        }
    });
}
