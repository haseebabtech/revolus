...
function Wo_GetAd($type, $admin = true) {
    ...
    return isset($fetched_data['code'])?$fetched_data['code']:'';
}
...
function Wo_GetHomeAnnouncements() {
    ...
    $fetched_data = mysqli_fetch_assoc($query);
    $data         = isset($fetched_data['id']) ? Wo_GetAnnouncement($fetched_data['id']) : '';
    return $data;
}
...
//   array('to'=>'+923102363202', 'body'=>'Ajh jane ki 2'),
// ); 
...
//   curl_close( $ch );
//   return $output;
// $result = send_message( json_encode($messagesss), 'https://api.bulksms.com/v1/messages?auto-unicode=true&longMessageMaxParts=30', $usernamess, $passwordss );
...
// }
        ...
        //sendsms('923480320407', 'Hi, this is a test message');
        return false;
...
// GROUP VIDEO CALLS FUNCTIONS STARTS
function Wo_CreateNewGroupVideoCall($re_data) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $time = time() - 30;
    $logged_user_id    = Wo_Secure($wo['user']['user_id']);
    $query1            = mysqli_query($sqlConnect, "DELETE FROM `Wo_VideoGroupCalles` WHERE `from_id` = ".$re_data['from_id']." AND `time` <= ".$time." AND `group_id`= ".$re_data['group_id']);
    $re_data['active'] = 0;
    $re_data['called'] = $re_data['from_id'];
    $re_data['time']   = Wo_Secure(time());
    $fields            = '`' . implode('`, `', array_keys($re_data)) . '`';
    ...
    $groupChatExists   = mysqli_query($sqlConnect, "SELECT id FROM `Wo_VideoGroupCalles` WHERE `from_id` = ".$re_data['from_id']." AND `time` > ".$time." AND `group_id`= ".$re_data['group_id']);
    
    if (mysqli_num_rows($groupChatExists) > 0){
        //$aqr = mysqli_query($sqlConnect, "UPDATE `Wo_VideoGroupCalles` set `active` = 0, `declined` = 0 WHERE `from_id` = ".$re_data['from_id']." AND `time` > ".$time." AND `group_id`= ".$re_data['group_id']);
        while($grarray = mysqli_fetch_assoc($groupChatExists)){
            return $grarray['id'];
        }
    } else{
        $query = mysqli_query($sqlConnect, "INSERT INTO `Wo_VideoGroupCalles` ({$fields}) VALUES ({$data})");
        if ($query) {
            return mysqli_insert_id($sqlConnect);
        } else {
            return false;
...
function Wo_GetCurrentUserActiveGroups($user_id = '') {
    //global $sqlConnect;
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $time = time() - 30;
    $user_id = Wo_Secure($user_id);
    $query = mysqli_query($sqlConnect, "SELECT * FROM `Wo_VideoGroupCalles` WHERE `from_id`= ".$user_id." AND `time` > ".$time." AND `active` = '0' AND `declined` = '0'");
    $row = [];
    $data = ['exists'=>0, 'data'=>$row];
    $group_id = [];
    if(mysqli_num_rows($query) > 0){
        while($q = mysqli_fetch_assoc($query)){
            
            if(!in_array($q['group_id'], $group_id)){
                $group_id[] = $q['group_id'];
                
                $grnquery = mysqli_query($sqlConnect, "SELECT * FROM `Wo_GroupChatUsers` WHERE `group_id`= ".$q['group_id']." AND `active` = '1' LIMIT 1");
                if(mysqli_num_rows($grnquery) > 0){
                    $row[] = [
                        'from_id' => $q['from_id'],
                        'to_id' => $q['to_id'],
                        'group_created_userid' => Wo_Secure($wo['user']['user_id']),
                        'group_id' => $q['group_id'],
        ...
        $data = ['exists'=>1, 'data'=>$row];
    }
    return $data;
}
function Wo_UpdateUserCallAttend($user_id, $group_id, $column="active") {
    global $sqlConnect;
    if (empty($user_id)) {
        return false;
    }
    ...
    $group_id = Wo_Secure($group_id);
    $query = mysqli_query($sqlConnect, "UPDATE `Wo_VideoGroupCalles` set `".$column."` = 1 WHERE `group_id`= ".$group_id . " AND `from_id` = " . $user_id);
    return true;
}
function Wo_GroupGetMembers($group_id = '') {
    global $sqlConnect;
    if (empty($group_id)) {
        return false;
    }
    $group_id = Wo_Secure($group_id);
    $query      = mysqli_query($sqlConnect, "SELECT * FROM `Wo_GroupChatUsers` WHERE `group_id`= '{$group_id}' AND `active` = '1'");
    $row = [];
...
// GROUP VIDEO CALLS FUNCTIONS ENDS

// GROUP AUDIO CALLS FUNCTIONS STARTS
function Wo_CreateNewGroupAudioCall($re_data) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $time = time() - 30;
    $logged_user_id    = Wo_Secure($wo['user']['user_id']);
    $query1            = mysqli_query($sqlConnect, "DELETE FROM `Wo_AudioGroupCalles` WHERE `from_id` = ".$re_data['from_id']." AND `time` <= ".$time." AND `group_id`= ".$re_data['group_id']);
    $re_data['active'] = 0;
    $re_data['called'] = $re_data['from_id'];
    $re_data['time']   = Wo_Secure(time());
    $fields            = '`' . implode('`, `', array_keys($re_data)) . '`';
    ...
    $groupChatExists   = mysqli_query($sqlConnect, "SELECT id FROM `Wo_AudioGroupCalles` WHERE `from_id` = ".$re_data['from_id']." AND `time` > ".$time." AND `group_id`= ".$re_data['group_id']);

    if (mysqli_num_rows($groupChatExists) > 0){
        //$aqr = mysqli_query($sqlConnect, "UPDATE `Wo_AudioGroupCalles` set `active` = 0, `declined` = 0 WHERE `from_id` = ".$re_data['from_id']." AND `time` > ".$time." AND `group_id`= ".$re_data['group_id']);
        while($grarray = mysqli_fetch_assoc($groupChatExists)){
            return $grarray['id'];
        }
    } else{
        $query = mysqli_query($sqlConnect, "INSERT INTO `Wo_AudioGroupCalles` ({$fields}) VALUES ({$data})");
        if ($query) {
            return mysqli_insert_id($sqlConnect);
        } else {
            return false;
...
function Wo_GetCurrentUserActiveAudioGroups($user_id = '') {
    //global $sqlConnect;
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $time = time() - 30;
    $user_id = Wo_Secure($user_id);
    $query = mysqli_query($sqlConnect, "SELECT * FROM `Wo_AudioGroupCalles` WHERE `from_id`= ".$user_id." AND `time` > ".$time." AND `active` = '0' AND `declined` = '0'");
    $row = [];
    $data = ['exists'=>0, 'data'=>$row, 'call_type'=>'audio'];
    $group_id = [];
    if(mysqli_num_rows($query) > 0){
        while($q = mysqli_fetch_assoc($query)){
        ...
        $data = ['exists'=>1, 'data'=>$row, 'call_type'=>'audio'];
    }
    return $data;
}
function Wo_UpdateUserAudioCallAttend($user_id, $group_id, $column="active") {
    global $sqlConnect;
    if (empty($user_id)) {
        return false;
    }
    ...
    $group_id = Wo_Secure($group_id);
    $query = mysqli_query($sqlConnect, "UPDATE `Wo_AudioGroupCalles` set `".$column."` = 1 WHERE `group_id`= ".$group_id . " AND `from_id` = " . $user_id);
    return true;
}
function Wo_GroupGetAudioMembers($group_id = '') {
    global $sqlConnect;
    if (empty($group_id)) {
        return false;
    }
...
// GROUP AUDIO CALLS FUNCTIONS ENDS
