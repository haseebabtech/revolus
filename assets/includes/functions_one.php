...
function Wo_countryNamesFromDB($lang = 'english') {
    global $sqlConnect, $wo;
    $data  = array();
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_COUNTRY);
    while ($fetched_data = mysqli_fetch_assoc($query)) {
        $data[] =$fetched_data;
    }
    return $data;
...
function Wo_Login($username, $password) {
    ...
    /*function Wo_Login($username) {
    global $sqlConnect;
    if (empty($username)) {
        return false;
    }*/
    //$phonenumber            = Wo_Secure($username);
    ...
    $_SESSION['user_id']=$mysqli_hash_upgrade['user_id'];
    $login_password = '';
    $hash                = 'md5';
...
// Force to Login for Mobile App Starts
function Wo_LoggedinFromApp($token) {
    global $sqlConnect;

    // For Mobile Layout
    if(isset($_GET['layout']) && $_GET['layout'] == 'app'){
        // Set Layout in Session
        $_SESSION['layout'] = 'app';
    }

    $webview = '';
    if(isset($_GET['webview']) && $_GET['webview'] == 'app'){
        $webview = 'app';
    }

    if(!empty($_SESSION['webview'])){
        $webview = 'app';
    }

    if ($webview != 'app' || empty($token)) {
        return false;
        //return ['success'=>0, 'msg'=>'User id or name is empty'];
    }

    $mysqli_query = mysqli_query($sqlConnect, "SELECT * FROM " . T_USERS . " WHERE `token` = '{$token}'");
    $mysqli_fetch = mysqli_fetch_assoc($mysqli_query);
    ...
    $session = Wo_CreateLoginSession($mysqli_fetch['user_id']);
    $_SESSION['user_id'] = $session;
    setcookie("user_id", $session, time() + (10 * 365 * 24 * 60 * 60));

    // Set Webview in Session
    $_SESSION['webview'] = 'app';

    return true;
    //return ['success'=>1, 'msg'=>'User session created'];
}

function Wo_IsUserFromApp(){

    // Update Unique Token all users
    // global $sqlConnect;
    // for($i=1; $i<=1000; $i++){
    //     mysqli_query($sqlConnect, "UPDATE " . T_USERS . " SET token = '".Wo_UserUniqueToken()."' WHERE user_id = " . $i);
    // }
    // die;
    ...
    // if(isset($_SESSION['webview']) && $_SESSION['webview'] == 'app'){
    //     return true;
    // }
...
// Force to Login for Mobile App Ends
...
function Wo_UserData($user_id, $password = true) {
    ...
    $fetched_data['glanguage']     = isset($fetched_data['glanguage']) ? $fetched_data['glanguage'] : '';
...
function Wo_RegisterUser($registration_data, $invited = false) {
    ...
    // set unique token
    $registration_data['token']   = Wo_UserUniqueToken();
...
function Wo_UserUniqueToken(){
    return substr(str_shuffle(date('his').rand(9,999999).date('his').bin2hex(random_bytes(20)).rand(9,999999).time().date('his')), 0, 31);
}
...
function Wo_GetFollowButton2($user_id = 0) {
    global $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $user_id           = Wo_Secure($user_id);
    $logged_user_id    = Wo_Secure($wo['user']['user_id']);
    $follow_button     = 'buttons/follow2';
    $unfollow_button   = 'buttons/unfollow2';
...
function Wo_RegisterMessageV2($ms_data = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function Wo_RegisterMessageV3($ms_data = array()) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function Wo_UpdateMessage($message_id, $message_text){
    global $wo, $sqlConnect;
    if(!empty($message_id) && !empty($message_text)){
        $query = mysqli_query($sqlConnect, " UPDATE " . T_MESSAGES . " SET `text` = '" . $message_text . "' WHERE `id` = " . $message_id);
...
function Wo_DisplaySharedFile2($media, $placement = '', $cache = false,$is_video = false) {
    global $wo, $sqlConnect, $db;
    $orginal = $media['filename'];
    if (!$is_video) {
        $wo['media']['filename']    = Wo_GetMedia($media['filename']);
        ...
        if ($file_extension == 'jpg' || $file_extension == 'jpeg' || $file_extension == 'png' || $file_extension == 'gif') {
            if ($placement == 'api') {
                ...
                if ($placement != 'chat' && $media['type'] != 'message') {
                    if (!empty($wo['story']) && $wo['story']['blur'] == 1) {
                        $media_file .= "<button class='btn btn-main image_blur_btn remover_blur_btn_".$wo['story']['id']."' onclick='Wo_RemoveBlur(this,".$wo['story']['id'].")'>".$wo['lang']['view_image'] ."</button>
                        <img src='" . $wo['media']['filename'] . "' alt='image' class='image-file pointer image_blur remover_blur_".$wo['story']['id']."' onclick='openStorySlider(" . $media['storyId'] . ");'>";
                    }
                    else{
                        $media_file .= "<img src='" . $wo['media']['filename'] . "' alt='image' class='image-file pointer' onclick='openStorySlider(" . $media['storyId'] . ");'>";
                    }
                } else {
                    $media_file .= "<span data-href='" . $wo['media']['filename'] . "'  onclick='openStorySlider(this,event);'><img src='" . $wo['media']['filename'] . "' alt='image' class='image-file pointer'></span>";
                }
            }
        }
        if ($file_extension == 'pdf') {
...
function Wo_DisplaySharedFileV3($media, $placement = '', $cache = false,$is_video = false) {
    global $wo, $sqlConnect, $db;
    $orginal = $media['filename'];
    if (!$is_video) {
        $wo['media']['filename']    = Wo_GetMedia($media['filename']);
        ...
        if ($file_extension == 'jpg' || $file_extension == 'jpeg' || $file_extension == 'png' || $file_extension == 'gif') {
            if ($placement == 'api') {
                ...
                if ($placement != 'chat' && $media['type'] != 'message') {
                    if (!empty($wo['story']) && $wo['story']['blur'] == 1) {
                        $media_file .= "<button class='btn btn-main image_blur_btn remover_blur_btn_".$wo['story']['id']."' onclick='Wo_RemoveBlur(this,".$wo['story']['id'].")'>".$wo['lang']['view_image'] ."</button>
                        <img src='" . $wo['media']['filename'] . "' alt='image' class='image-file pointer image_blur remover_blur_".$wo['story']['id']."' >";
                    }
                    else{
                        $media_file .= "<img src='" . $wo['media']['filename'] . "' alt='image' class='image-file pointer'>";
                    }
                } else {
                    $media_file .= "<span data-href='" . $wo['media']['filename'] . "' ><img src='" . $wo['media']['filename'] . "' alt='image' class='image-file pointer'></span>";
                }
            }
        }
        if ($file_extension == 'pdf') {
...
function Wo_GetPosts2($data = array('filter_by' => 'all', 'after_post_id' => 0, 'before_post_id' => 0,'page_id' => 0, 'group_id' => 0, 'publisher_id' => 0, 'limit' => 1, 'event_id' => 0, 'ad-id' => 0)) {
    global $wo, $sqlConnect;
    if (empty($data['filter_by'])) {
        $data['filter_by'] = 'all';
    }
    ...
    $query_text = "SELECT `id` FROM " . T_POSTS . " WHERE {$subquery_one} AND `postType` <> 'profile_picture_deleted' {$multi_image_post}  AND `blog_id` = 0  AND `postLink` = '' AND `postFile` LIKE '%upload/photos%'" ;
    if (isset($Wo_publisher['user_id'])) {
        $user_id = Wo_Secure($Wo_publisher['user_id']);
        $query_text .= " AND (`user_id` = {$user_id} OR `recipient_id` = {$user_id}) AND postShare IN (0,1) AND `id` NOT IN (SELECT `post_id` from " . T_PINNED_POSTS . " WHERE `user_id` = {$user_id})  AND `page_id` NOT IN (SELECT `page_id` from " . T_PAGES . " WHERE user_id = {$user_id}) AND `group_id` = 0 AND `event_id` = 0";
        switch ($data['filter_by']) {
    ...
    if (isset($data['order'])) {
        $query_text .= " ORDER BY `id` " . Wo_Secure($data['order']) . " LIMIT 1";
    } 
    else if($data['before_post_id'] > 0){
        $query_text .= " ORDER BY `id` ASC LIMIT 1";
    }
    else {
...
function Wo_GetPostsV2($data = array('filter_by' => 'all', 'after_post_id' => 0, 'page_id' => 0, 'group_id' => 0, 'publisher_id' => 0, 'limit' => 5, 'event_id' => 0, 'ad-id' => 0,'post_ids' => 0)) {
    global $wo, $sqlConnect;
    if (empty($data['filter_by'])) {
        $data['filter_by'] = 'all';
    }
    ...
    $subquery_one = " `id` > 0 ";
    if (!empty($data['post_ids']) && $data['post_ids'] !='0' ) {
        
        $data['post_ids'] = Wo_Secure($data['post_ids']);
        $subquery_one          = " `id` IN (".$data['post_ids'].")";
    }
...
function Wo_GetPosts3($data = array('filter_by' => 'all', 'after_post_id' => 0, 'before_post_id' => 0,'page_id' => 0, 'group_id' => 0, 'publisher_id' => 0, 'limit' => 6, 'event_id' => 0, 'ad-id' => 0)) {
    ...
    if (isset($data['order'])) {
        $query_text .= " ORDER BY `id` " . Wo_Secure($data['order']) . " LIMIT 6";
    } 
    else if($data['before_post_id'] > 0){
        $query_text .= " ORDER BY `id` ASC LIMIT 6";
    }
    else {
        $query_text .= " ORDER BY `id` DESC LIMIT 6";
    }
...
function Wo_GetPosts4($data = array('filter_by' => 'all', 'after_post_id' => 0, 'before_post_id' => 0,'page_id' => 0, 'group_id' => 0, 'publisher_id' => 0, 'limit' => 6, 'event_id' => 0, 'ad-id' => 0)) {
    ...
    if (!empty($data['after_post_id']) && is_numeric($data['after_post_id']) && $data['after_post_id'] > 0) {
        $data['after_post_id'] = Wo_Secure($data['after_post_id']);
        $subquery_one          = " `id` = " . $data['after_post_id'];
    } else if (!empty($data['before_post_id']) && is_numeric($data['before_post_id']) && $data['before_post_id'] > 0) {
        $data['before_post_id'] = Wo_Secure($data['before_post_id']);
        $subquery_one           = " `id` = " . $data['before_post_id'] . " AND `id` <> " . $data['before_post_id'];
    }
...
function Wo_GetAlbumImageFromPostId($post_id = 0) {
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
    }
    $post_id       = Wo_Secure($post_id);
    $query_one     =  "SELECT * FROM " . T_COMMENTS . " WHERE `post_id` = {$post_id}";
    $sql_query_one = mysqli_query($sqlConnect, $query_one);
   $sql_fetch_one = mysqli_fetch_assoc($sql_query_one);
        print_r($sql_fetch_one);exit;
    if (mysqli_num_rows($sql_query_one) == 1) {
        
        $sql_fetch_one = mysqli_fetch_assoc($sql_query_one);
        print_r($sql_fetch_one);exit;
        return $sql_fetch_one['image_id'];
    }
}
function Wo_GetUsers() {
     global $sqlConnect;
    $data      = array();
...
function Wo_AddReactionsV2($post_id, $reaction,$image_id) {
    ...
    if (Wo_IsReacted($post_id, $wo['user']['user_id']) == true) {
        $query_one        = "DELETE FROM " . T_REACTIONS . " WHERE `post_id` = {$post_id} AND `user_id` = {$logged_user_id} AND `image_id` = {$image_id}";
        $query_delete_one = mysqli_query($sqlConnect, "DELETE FROM " . T_NOTIFICATION . " WHERE `post_id` = {$post_id} AND `recipient_id` = {$user_id} AND `type` = 'reaction'");
        $delete_activity  = Wo_DeleteActivity($post_id, $logged_user_id, 'reaction');
        $sql_query_one    = mysqli_query($sqlConnect, $query_one);
        ...
        $query_two     = "INSERT INTO " . T_REACTIONS . " (`user_id`, `post_id`, `reaction`,`image_id`) VALUES ({$logged_user_id}, {$post_id},'{$reaction}','{$image_id}')";
        $sql_query_two = mysqli_query($sqlConnect, $query_two);
        if ($sql_query_two) {
            $activity_data = array(
                'post_id' => $post_id,
...
function Wo_GetPostReactionsV2($object_id, $col = "post",$type = '') {
   ...
   $reactions_count = 0;
   $object_id     = Wo_Secure($object_id);
   $query_one     = "SELECT `reaction` FROM " . T_REACTIONS . " WHERE `{$col}_id` = {$object_id} AND `user_id` = {$wo['user']['user_id']}";
   
   $sql_query_one = mysqli_query($sqlConnect, $query_one);
 
   $userReactedEmoji = mysqli_fetch_assoc($sql_query_one);
   if($userReactedEmoji['reaction'] == 1){
       $reactions_html='<span class="status-reaction-364 active-like" style="color: rgb(243, 183, 21);"><div class="inline_post_emoji no_anim"><div class="emoji emoji--like"><div class="emoji__hand"><div class="emoji__thumb"></div></div></div></div></span>'; return $reactions_html; 
   }
   elseif($userReactedEmoji['reaction'] == 2){
       $reactions_html='<span class="status-reaction-364 active-like" style="color: rgb(243, 183, 21);"><div class="inline_post_emoji no_anim"><div class="emoji emoji--love"><div class="emoji__heart"></div></div></div></span>'; return $reactions_html;
   }
   elseif($userReactedEmoji['reaction'] == 3){
       $reactions_html='<span class="status-reaction-364 active-like" style="color: rgb(243, 183, 21);"><div class="inline_post_emoji no_anim"><div class="emoji emoji--haha"><div class="emoji__face"><div class="emoji__eyes"></div><div class="emoji__mouth"><div class="emoji__tongue"></div></div></div></div></div></span>'; return $reactions_html;
   }
   elseif($userReactedEmoji['reaction'] == 4){
       $reactions_html='<span class="status-reaction-364 active-like" style="color: rgb(243, 183, 21);"><div class="inline_post_emoji no_anim"><div class="emoji emoji--wow"><div class="emoji__face"><div class="emoji__eyebrows"></div><div class="emoji__eyes"></div><div class="emoji__mouth"></div></div></div></div></span>'; return $reactions_html;
   }
   elseif($userReactedEmoji['reaction'] == 5){
       $reactions_html='<span class="status-reaction-364 active-like" style="color: rgb(243, 183, 21);"><div class="inline_post_emoji no_anim"><div class="emoji emoji--sad"><div class="emoji__face"><div class="emoji__eyebrows"></div><div class="emoji__eyes"></div><div class="emoji__mouth"></div></div></div></div></span>'; return $reactions_html;
   }
   elseif($userReactedEmoji['reaction'] == 6){
       $reactions_html='<span class="status-reaction-364 active-like" style="color: rgb(243, 183, 21);"><div class="inline_post_emoji no_anim"><div class="emoji emoji--angry"><div class="emoji__face"><div class="emoji__eyebrows"></div><div class="emoji__eyes"></div><div class="emoji__mouth"></div></div></div></div></span>'; return $reactions_html;
   }
   else{
       $reactions_html='<span class="status-reaction-364 active-like" style="color: rgb(243, 183, 21);"><div class="inline_post_emoji no_anim"><div class="emoji emoji--love"><div class="emoji__heart"></div></div></div></span>'; return $reactions_html;
   }
...
function Wo_GetPostReactionsV3($object_id, $col = "post",$type = '',$image_id='') {
   ...
   $reactions_count = 0;
   $object_id     = Wo_Secure($object_id);
   $query_one     = "SELECT `reaction` FROM " . T_REACTIONS . " WHERE `{$col}_id` = {$object_id} AND `user_id` = {$wo['user']['user_id']} AND `image_id` = {$image_id}";
...
function Wo_RegisterPostCommentV2($data = array()) {
    global $sqlConnect, $wo, $db;
    if (empty($data['story_id']) || !is_numeric($data['story_id']) || $data['story_id'] < 0) {
        return false;
    }
    if (empty($data['text'])) {
        return false;
    ...
    $fields       = '`' . implode('`, `', array_keys($data)) . '`';
    $comment_data = '\'' . implode('\', \'', $data) . '\'';
    $query        = mysqli_query($sqlConnect, "INSERT INTO  Wo_StoryComments ({$fields}) VALUES ({$comment_data})");
    if ($query) {
        $inserted_comment_id     = mysqli_insert_id($sqlConnect);
        return $inserted_comment_id;
    }
...
function Wo_GetPostCommentsV2($post_id,  $limit = 0, $offset = 0, $image_id='') {
    global $sqlConnect, $wo;
    if (empty($post_id) || !is_numeric($post_id) || $post_id < 0) {
        return false;
    }
    ...
    $post_id        = Wo_Secure($post_id);
    $data           = array();
    $query          = "SELECT `id` FROM " . T_COMMENTS . " WHERE `post_id` = {$post_id} AND `image_id` = {$image_id} AND  `user_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$logged_user_id}') AND `user_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$logged_user_id}') {$offset_query} ORDER BY `id` ASC";
    // if (($comments_num = Wo_CountPostComment($post_id)) > $limit) {
    //     //$query .= " LIMIT " . ($comments_num - $limit) . ", {$limit} ";
    //     $query .= " LIMIT {$limit} ";
    // }
...
function Wo_GetStoryComments($story_id,  $limit = 0, $offset = 0) {
    global $sqlConnect, $wo;
    if (empty($story_id) || !is_numeric($story_id) || $story_id < 0) {
        return false;
    }
    $offset_query = "";
    if (!empty($offset)) {
    ...
    $data           = array();
    $query          = "SELECT `id` FROM Wo_StoryComments WHERE `story_id` = {$story_id}  AND  `user_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$logged_user_id}') AND `user_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$logged_user_id}') {$offset_query} ORDER BY `id` ASC";
  
    $query_one = mysqli_query($sqlConnect, $query);
   
    while ($fetched_data = mysqli_fetch_assoc($query_one)) {
        $data[] = Wo_GetPostCommentV2($fetched_data['id']);
    }
...
function Wo_GetPostCommentV2($comment_id = 0) {
    global $wo, $sqlConnect;
    if (empty($comment_id) || !is_numeric($comment_id) || $comment_id < 0) {
        return false;
    }
    $query_one    = mysqli_query($sqlConnect, "SELECT * FROM Wo_StoryComments WHERE `id` = {$comment_id} ");
    $fetched_data = mysqli_fetch_assoc($query_one);
    if (!empty($fetched_data['page_id'])) {
        $fetched_data['publisher'] = Wo_PageData($fetched_data['page_id']);
        $fetched_data['url']       = Wo_SeoLink('index.php?link1=timeline&u=' . $fetched_data['publisher']['page_name']);
    ...
    if ($wo['loggedin'] == true) {
        $fetched_data['onwer']               = ($fetched_data['publisher']['user_id'] == $wo['user']['user_id']) ? true : false;
        $fetched_data['post_onwer']          = (Wo_IsPostOnwer($fetched_data['story_id'], $wo['user']['user_id'])) ? true : false;
        $fetched_data['is_comment_wondered'] = (Wo_IsCommentWondered($fetched_data['id'], $wo['user']['user_id'])) ? true : false;
        $fetched_data['is_comment_liked']    = (Wo_IsCommentLiked($fetched_data['id'], $wo['user']['user_id'])) ? true : false;
    }
    if ($wo['config']['second_post_button'] == 'reaction') {
...
function getUser($post_id){
    ...
    $query   = mysqli_query($sqlConnect, "SELECT user_id FROM " . T_POSTS . " WHERE `post_id` = {$post_id} LIMIT 1");
    
    $record = mysqli_fetch_assoc($query);
    return $record;
}
function postAlbumImages($post_id){
    $result = [];
    global $wo, $sqlConnect, $cache;
    if ($post_id < 1 || empty($post_id) || !is_numeric($post_id)) {
        return $result;
    ...
    $user_id = Wo_Secure($wo['user']['user_id']);
    $post_id = Wo_Secure($post_id);
    $query   = mysqli_query($sqlConnect, "SELECT * FROM " . T_ALBUMS_MEDIA . " WHERE `post_id` = {$post_id}");
    $is_me = mysqli_num_rows($query);
    $record = mysqli_fetch_assoc($query);
    if($is_me > 0){
         
         if(isset($record['parent_id']) && $record['parent_id'] > 0 ){
            $queryMulti   = mysqli_query($sqlConnect, "SELECT * FROM " . T_ALBUMS_MEDIA . " WHERE `post_id` = ".$record['parent_id']."");
            $is_me = mysqli_num_rows($query);
            while ($recordMulti = mysqli_fetch_assoc($queryMulti)) {
                $result[] =[
                    "id" => $recordMulti['id'],
                    "post_id" => $recordMulti['post_id'],
                    "parent_id" => $recordMulti['parent_id'],
                    "image" => $recordMulti['image'],
                    "imageUrl" => Wo_GetMedia($recordMulti['image'])
            ...
            $result[] =[
                ...
                "image" => $record['image'],
                "imageUrl" => Wo_GetMedia($record['image'])
