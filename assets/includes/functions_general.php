...
function Wo_LanguageTranslate($content, $detect_language, $translate_language){
    global $wo;

    $tr = new GoogleTranslate();

    if(!empty($detect_language)){
        $tr->setSource($detect_language);
    } else{
        $tr->setSource(); // Detect language automatically
    }

    $language = 'en';
    if(!empty($translate_language)){
        $language = $translate_language;
    }

    if(isset($wo['user']) && !empty($wo['user']['translate_language'])){
        
        $language = $wo['user']['translate_language'];
        
    }
    $tr->setTarget($language);
    // echo $content;
    // echo " after trnaslate ";
    // echo  $tr->translate($content);exit;
    
    return !empty($content) ? $tr->translate($content) : $content;
}
...
function Wo_SeoLink($query = '') {
    ...
    if ($wo['config']['seoLink'] == 1) {
        $query = preg_replace(array(
            ...
            '/^index\.php\?link1=authorizepay$/i',
            '/^index\.php\?link1=payment$/i',
            ...
            $config['site_url'] . '/authorizepay/',
            $config['site_url'] . '/authorizepay/payment/$1/',
