...
if (!empty($_POST['type'])) {
	...
	if (in_array('following', $types)) {
		...
		foreach ($following as $key2 => $user_list) {
			...
			if(isset($following[$key2]['username'])){
				$following[$key2]['username'] = Wo_LanguageTranslate($following[$key2]['username'], "", "");
			}
			if(isset($following[$key2]['name'])){
				$following[$key2]['name'] = Wo_LanguageTranslate($following[$key2]['name'], "", "");
			}
