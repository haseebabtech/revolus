...
//if(isset($_FILES['postFile']) && isset($_FILES['postFile']['name'])){
    // $apiLogData = "\n\n\n";
    // $apiLogData = date("Y-m-d H:i:s")." - ".json_encode($_FILES);
//}

$apiLogData = "\n\n\n";
$apiLogData = date("Y-m-d H:i:s")." - ".json_encode($_FILES);

$file = fopen("logs_new_post.txt", "a");
fwrite($file, "\n\n". $apiLogData );
fclose($file);
...
if (isset($_FILES['postPhotos']['name'])) {
    ...
    for ($i = 0; $i < count($_FILES['postPhotos']['name']); $i++) {
        ...
        } elseif(isset($_FILES['postPhotos']['name'][0])){
...
if (empty($error_message)) {
    ...
    $post_data = array(
        ...
        'postPrivacy' => 0, //Wo_Secure($post_privacy),
