...
foreach ($get_all_stories as $key => $one_story) {
	...
	foreach ($get_stories as $key => $story) {
        ...
        if(isset($story['username'])){
            $story['username'] = Wo_LanguageTranslate($story['username'], $detect_language, $translate_language);
        }
        if(isset($story['name'])){
            $story['name'] = Wo_LanguageTranslate($story['name'], $detect_language, $translate_language);
        }
