...
if (!empty($_POST['type']) && in_array($_POST['type'], $required_fields)) {
    ...
    if ($_POST['type'] == 'create') {
        ...
        if (in_array(true, $request)) {
            $error_code    = 5;
            $error_message = 'Please check your details.1';
        } else {
            ...
            if (!in_array($_FILES["media"]["type"], $ad_media_types)) {
                ...
                $error_code    = 5;
                $error_message = 'Please check your details.2';
            } else if ($_POST['bidding'] != 'clicks' && $_POST['bidding'] != 'views') {
                $error_code    = 5;
                $error_message = 'Please check your details.3';
        ...
        if (empty($error_message)) {
            ...
            $create_session = mysqli_query($sqlConnect, "INSERT INTO " . T_USER_ADS . " (`name`,`url`,`headline`,`description`,`location`,`audience`,`gender`,`bidding`,`posted`,`appears`,`user_id`, `ad_media`) VALUES ('".Wo_Secure($_POST['name'])."', '".Wo_Secure($_POST['website'])."', '".Wo_Secure($_POST['headline'])."', '".Wo_Secure($_POST['description'])."', '".Wo_Secure($_POST['location'])."', '".Wo_Secure(implode(',', $_POST['audience-list']))."', '".Wo_Secure($_POST['gender'])."', '".Wo_Secure($_POST['bidding'])."', '".time()."', '".Wo_Secure($_POST['appears'])."', '".Wo_Secure($wo['user']['user_id'])."', '".$registration_data['ad_media']."')");
    ...
    if ($_POST['type'] == 'edit') {
        ...
        if (in_array(true, $request)) {
            $error_code    = 5;
            $error_message = 'Please check your details.4';
        } else {
            if (strlen($_POST['name']) < 3 || strlen($_POST['name']) > 100) {
                $error_code    = 6;
                $error_message = 'Please enter a valid company name!';
            ...
            if (!in_array($_POST['bidding'], array(
                ...
                $error_code    = 5;
                $error_message = 'Please check your details.5';
            }
            $img_types = array(
                'image/png',
                'image/jpeg',
