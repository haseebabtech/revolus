...
if (empty($error_code)) {
    ...
    if (!empty($data['events'])) {
	    ...
	    foreach ($get_events as $key => $event) {
			...
			if(isset($event['name'])){
				$event['name'] = Wo_LanguageTranslate($event['name'], "", "");
			}
            if(isset($event['description'])){
				$event['description'] = Wo_LanguageTranslate($event['description'], "", "");
			}
            if(isset($event['user_data']) && isset($event['user_data']['username'])){
				$event['user_data']['username'] = Wo_LanguageTranslate($event['user_data']['username'], "", "");
			}
            if(isset($event['user_data']) && isset($event['user_data']['name'])){
				$event['user_data']['name'] = Wo_LanguageTranslate($event['user_data']['name'], "", "");
			}
