...
if (empty($error_code)) {
    ...
    //print_r('THIS IS TEXT 404055');die;
    ...
    if (!empty($data['pro_users'])) {
    	...
    	if (!empty($pro_users)) {
            foreach ($pro_users as $key => $pro_user) {
                ...
                if(isset($pro_user['username'])){
                    $pro_user['username'] = Wo_LanguageTranslate($pro_user['username'], $detect_language, $translate_language);
                }
                if(isset($pro_user['name'])){
                    $pro_user['name'] = Wo_LanguageTranslate($pro_user['name'], $detect_language, $translate_language);
                }
    ...
    if (!empty($data['announcement'])) {
        ...
        if (!empty($response_data['announcement'])) {
            ...
            // Add Keys for API Response
            $response_data['announcement'] = [
                "id"=>"4",
                "text"=>"<p style=\"text-align: right;\">hhhhhhhhhhhhhhhhhhhhhhhhhhh</p>",
                "time"=>"1560004594",
                "active"=> "1",
                "text_decode"=>"hhhhhhhhhhhhhhhhhhhhhhhhhhh",
                "time_text"=>"3 seconds ago"
            ];
        }
    } 
    else{
